...
import { ValueInterface } from 'phpcore';

describe('Basic markdown usage integration (Synchronous mode)', () => {
    let dotPHP: DotPHPInterface;

    beforeEach(() => {
        dotPHP = (dotPHPFactory as DotPHPFactoryInterface).create(
            __dirname + '/fixtures/basic_usage/sync'
        );
    });

    it('should be able to render a parsed Markdown tree containing just plain text', () => {
        const resultValue = dotPHP.evaluate(
            `
<?php
